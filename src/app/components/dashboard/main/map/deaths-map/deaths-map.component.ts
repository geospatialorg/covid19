...
@Component({
  selector: 'app-deaths-map',
  templateUrl: './deaths-map.component.html',
  styleUrls: ['./deaths-map.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class DeathsMapComponent implements OnInit {
  map: Map = null;
  ...
  drawFeatures(source, geojsonFeatures, iconStyle) {
    let maxDeaths = this.mapData.sort((a, b) => {
      return b.total_dead - a.total_dead;
    });

    this.mapData.map(e => {
      let data = geojsonFeatures.find(f => e.county_code === f.get('county_code'));

      if(data && e.total_dead > 0) {
        let radius = this.linearInterpolate(e.total_dead, maxDeaths[maxDeaths.length-1].total_dead, maxDeaths[0].total_dead, this.minRadius, this.maxRadius);
