...
@Component({
  selector: 'app-healed-map',
  templateUrl: './healed-map.component.html',
  styleUrls: ['./healed-map.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class HealedMapComponent implements OnInit {
  map: Map = null;
  ...
  drawFeatures(source, geojsonFeatures, iconStyle) {
    let maxHealed = this.mapData.sort((a, b) => {
      return b.total_healed - a.total_healed;
    });

    this.mapData.map(e => {
      let data = geojsonFeatures.find(f => e.county_code === f.get('county_code'));

      if(data && e.total_healed > 0) {
        let radius = this.linearInterpolate(e.total_healed, maxHealed[maxHealed.length - 1].total_healed, maxHealed[0].total_healed, this.minRadius, this.maxRadius);
