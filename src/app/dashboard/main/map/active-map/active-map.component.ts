...
export class ActiveMapComponent implements OnInit {
  ...
  drawFeatures(source, geojsonFeatures, iconStyle) {
    let maxActives = this.mapData.sort((a, b) => {
    ...
    this.mapData.map(e => {
      ...
      if(data && e.total_active > 0) {
        let radius = this.linearInterpolate(e.total_active, maxActives[maxActives.length-1].total_active, maxActives[0].total_active, this.minRadius, this.maxRadius);
